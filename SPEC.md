...
Container menu items show the container's real icon and color, using the same `class="menuitem-iconic"` + `image` attribute mechanism Zen's own "open in" dropdown already uses in this exact dialog (a `<menulist>` doesn't automatically mirror a selected `<menuitem>`'s icon onto its own closed/collapsed display, so the closed menulist's `image` is also re-applied explicitly on every selection change, not left to implicit syncing).

Getting the *color* right took three attempts, all recorded here since the failure modes are non-obvious:

1. Setting the container's color directly via the element's `color` property: works for the icon (the existing `menulist[image]::part(icon) { fill: currentColor; }` rule resolves `currentColor` to it), but `color` also inherits to the label text and dropdown arrow, retinting the *entire* control to match the container — they need to stay the dialog's standard color, same as the Space dropdown.
2. Scoping a dedicated `--zsr-icon-color` custom property via a rule injected as a `<style>` element: the custom property never reached the icon at all, because a plain `<style>` element is inert in this XUL/XHTML window — it's just dead markup, never parsed as a stylesheet. (The dialog's real stylesheets all load via `<link rel="stylesheet">` inside `<linkset>`.)
3. Re-injecting the same rule as a `<link rel="stylesheet" href="data:text/css,...">` instead: still didn't render any color, suggesting `-moz-context-properties`/`context-fill` resolution for a dynamically-added `<menuitem>`/`<menulist>` `image` doesn't reliably pick up a custom-property-driven `fill` in this context.

The mechanism that actually ships: the icon's source SVG (Firefox's own container artwork, vendored verbatim — see `IDENTITY_ICON_SVGS`) ships with `fill="context-fill"`, a placeholder meant to be resolved via `-moz-context-properties`. Instead of relying on that resolving correctly, the real hex color is substituted directly into the SVG text before it's ever used as an `image` value, and the result is encoded as a `data:image/svg+xml,...` URI. The icon is then just a plain, already-colored image — no CSS cascade, custom property, or `-moz-context-properties` involved at all, and nothing touches the menulist's own `color`, so the label and arrow are unaffected.
